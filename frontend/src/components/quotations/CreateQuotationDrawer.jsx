...
export default function CreateQuotationDrawer({ open, onClose, onCreated, duplicateFrom, prefillCustomerId }) {
  ...
  const [terms, setTerms] = useState("Prices valid for 15 days. Payment as per agreed terms.");
  ...
  useEffect(() => {
    ...
    if (duplicateFrom) {
      // Prefill from an existing quotation. Rate/GST/HSN are re-derived from
      // each product's current values (not the historical snapshot), since
      // the backend always recomputes them from the live Product anyway.
      setCustomer(duplicateFrom.customer?._id ?? "");
      ...
      setItems(
        duplicateFrom.items.map((item) => ({
          ...
          rate: item.product?.price ?? item.rate,
          gst: item.product?.gst ?? item.gst,
        }))
      );
      setNotes(duplicateFrom.notes ?? "");
      setTerms(duplicateFrom.termsAndConditions ?? terms);
    } else {
  ...
  }, [open, duplicateFrom, prefillCustomerId]);
  ...
  return (
    <SideDrawer
      open={open}
      onClose={onClose}
      title={duplicateFrom ? `Duplicate ${duplicateFrom.quotationNumber}` : "Create Quotation"}
      subtitle="Build a quotation with GST-aware line items."
      footer={
        <div className="flex justify-end gap-2">
          ...
          <div className="w-40">
            <Button type="submit" form="create-quotation-form" loading={saving}>
              Save Quotation
            </Button>
          </div>
        </div>
    ...
    >
      <form id="create-quotation-form" onSubmit={handleSubmit} className="space-y-5" noValidate>
        <div className="grid grid-cols-3 gap-3">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-gray-500">Quotation Date</label>
            <input
              type="date"
              className="w-full rounded-lg border border-gray-200 px-2.5 py-2 text-sm"
              value={quotationDate}
          ...
          <div className="space-y-2">
            ...
            {items.map((item, index) => (
              <div key={index} className="grid grid-cols-12 items-center gap-2">
                ...
                />
                {/* Rate/GST are read-only: backend/src/services/quotation.service.js
                    always recomputes them from the selected Product, ignoring
